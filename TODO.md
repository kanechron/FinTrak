...
- [ ] **Spending anomaly detection** — flag transactions that deviate from a merchant/category's historical mean+stddev (e.g. a normally $60-90 grocery run showing up at $340); rolling stats per merchant, computable incrementally as new transactions sync in. Note: LTE Forecasting's `CalculateDeviation` already computes this exact mean/stddev/z-score shape per category-month — this item may reduce to reusing that logic at the per-merchant/transaction grain rather than building it from scratch
