...
export function getTransactions(
  ...
  return api.get<Transaction[]>(
    `/transactions/get-transactions${query ? `?${query}` : ""}`,
  );
}

export function getTransactionsByCategory(id: string): Promise<Transaction[]> {
  return api.get<Transaction[]>(
    `/transactions/get-transactions-by-category/${id}`,
  );
}

export function addTransaction(
  transaction: TransactionAddPayload,
): Promise<void> {
...
export function applyCategoryByMerchant(
  ...
  return api.patch("transactions/apply-category-by-merchant", {
    merchantName,
    categoryId,
  });
}
