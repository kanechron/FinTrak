  ...
  <label className="flex items-center gap-3 text-sm text-gray-300 cursor-pointer mt-4">
    ...
    Recurring
      ...
      {isRecurring && (
        ...
        <select size={8} className="w-full p-2 bg-gray-800 border border-gray-700 rounded-lg text-sm text-gray-200 focus:outline-none focus:border-gray-500 max-h-48 overflow-y-auto" onChange ={e => setCategoryId(e.target.value || null)} value={categoryId || ''}>
          <option value="">All Categories</option>
          {categories.sort((a, b) => a.name.localeCompare(b.name)).map(c => (
            <option key={c.id} value={c.id}>{c.name.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, char => char.toUpperCase())}</option>
          ))}
        </select>
