...
export default function CategoryDetailCarousel({
  ...
  useEffect(() => {
    ...
    Promise.all(
      categories.map((c) =>
        getCategoryDetailSpending(c.id, from, to).then((data) => ({
          id: c.id,
          data,
        })),
      ),
  ...
  const displayData = [
    ...data.map((d) => ({
      ...d,
      name: d.name.startsWith(current.name + "_")
        ? d.name.slice(current.name.length + 1)
        : d.name,
    })),
    ...(uncategorized > 0.005
      ? [{ name: formatName(current.name), amount: uncategorized }]
      : []),
  ];
