...
const COLORS = ['#818cf8', '#34d399', '#f472b6', '#fb923c', '#38bdf8', '#a78bfa', '#facc15', '#4ade80']
const tooltipStyle = { backgroundColor: '#111827', border: '1px solid #1f2937', borderRadius: 8 }
const tooltipTextStyle = { color: '#e5e7eb' }
const formatName = (v: string) => v.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, x => x.toUpperCase())
...
export default function CategoryDetailCarousel({ categories, from, to }: Props) {
  ...
  const [detailCache, setDetailCache] = useState<Record<string, { name: string; amount: number }[]>>({})
    ...
    <section className="border border-gray-800 rounded-xl p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <span className="text-xs text-gray-500">{index + 1} / {categories.length}</span>
          <button
      ...
      {loading ? (
        ...
        <ResponsiveContainer width="100%" height={280}>
          <PieChart>
            <Pie data={displayData} dataKey="amount" nameKey="name" cx="50%" cy="50%" outerRadius={110}>
              {displayData.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
            </Pie>
            <Tooltip formatter={formatAmount} contentStyle={tooltipStyle} labelStyle={tooltipTextStyle} itemStyle={tooltipTextStyle} />
