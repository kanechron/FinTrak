        ...
        <ResponsiveContainer width="100%" height={280}>
          {chartType === 'Bar' ? (
            ...
            <AreaChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={axisStyle} tickFormatter={v => `$${v}`} />
              <Tooltip formatter={(v: unknown) => `$${(v as number).toFixed(2)}`} contentStyle={tooltipStyle} labelStyle={tooltipTextStyle} itemStyle={tooltipTextStyle} />
              <Area type="monotone" dataKey="amount" stroke="#818cf8" fill="#818cf8" fillOpacity={0.15} />
            </AreaChart>
          ) : (
            <LineChart data={chartData} margin={{ top: 4, right: 16, left: 0, bottom: 0 }}>
              ...
              <YAxis tick={axisStyle} tickFormatter={v => `$${v}`} />
              <Tooltip formatter={(v: unknown) => `$${(v as number).toFixed(2)}`} contentStyle={tooltipStyle} labelStyle={tooltipTextStyle} itemStyle={tooltipTextStyle} />
              <Line type="monotone" dataKey="amount" stroke="#818cf8" strokeWidth={2} dot={{ fill: '#818cf8', r: 3 }} />
            </LineChart>
          )}
        </ResponsiveContainer>
      )}
