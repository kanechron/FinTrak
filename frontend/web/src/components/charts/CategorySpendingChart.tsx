        ...
        <ResponsiveContainer width="100%" height={280}>
          {chartType === 'Bar' ? (
            <BarChart data={data} margin={{ top: 4, right: 16, left: 0, bottom: 60 }}>
              ...
              <XAxis dataKey="name" tick={{ fill: '#6b7280', fontSize: 11 }} angle={-35} textAnchor="end" interval={0} tickFormatter={formatName} />
            ...
            <PieChart>
              <Pie data={data} dataKey="amount" nameKey="name" cx="50%" cy="50%"
                outerRadius={110} innerRadius={chartType === 'Donut' ? 55 : 0}
              >
                {data.map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
              </Pie>
              <Tooltip formatter={formatAmount} contentStyle={tooltipStyle} labelStyle={tooltipTextStyle} itemStyle={tooltipTextStyle} />
