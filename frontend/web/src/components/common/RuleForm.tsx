...
export default function RuleForm({ onCancel }: Props) {
  // Rule-level fields
  ...
  return (
    <div className="flex flex-col gap-4 py-4 border-b border-line">
        ...
        {conditions.map((condition, i) => (
          <div key={condition.groupId} className="flex items-center gap-2">
            <select
              ...
              onChange={(e) => updateCondition(i, { conditionField: e.target.value as Condition['conditionField'] })}
              className={inputClass}
            >
        ...
        {actions.map((action, i) => (
          <div key={i} className="flex items-center gap-2">
            <select
              ...
              onChange={(e) => updateAction(i, { actionField: e.target.value as Action['actionField'] })}
              className={inputClass}
            >
              {ACTION_FIELDS.map((f) => (
                <option key={f} value={f}>
      ...
      <p className={errorClass}>{/* validation errors go here */}</p>

      <div className="flex justify-end gap-2">
        <button onClick={onCancel} className="text-sm text-ink-3 hover:text-ink-2 transition-colors px-4 py-2">
          Cancel
        </button>
        <button className={`${primaryButtonClass} !w-auto px-6`}>Save Rule</button>
      </div>
    </div>
  )
}
